...
dronet_data_con = torch.from_numpy(np.concatenate((dronet_data, test_imu_data),axis=1))
...
if __name__ == "__main__":
    ...
    Dronet.eval()"""
    #--------------------------------------------------------
    lstmf = LstmNet(input_size_f, output_size, lstmf_hidden_size, lstmf_num_layers)
    lstmf.to(device)
    print("LSTMf Model:", lstmf)
    lstmf.load_state_dict(torch.load('C:\\Users\\deepd\\OneDrive\\Masaüstü\\LSTMf\\model\\test_9_RESNET\\16\\16hidden_2_0.0001_1_loss_0.1786_PG.pth'))   
    lstmf.eval()
    ...
    P_t = np.eye(4)
    y = np.zeros((test_data_size,4),dtype=np.float32)
    ...
    for i in range(12,test_data_size):
        ...
        with torch.no_grad():
            ...
            lstm_inputs = dronet_data_con[i-time_stamp:i].reshape(1,12,11)
